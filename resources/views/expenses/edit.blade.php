    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 md:px-8">
        <div class="md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Edit {{ $expense->expense_number }} Expense</h2>
            </div>
        ...
        <form action="{{ route('expenses.update',$expense) }}" method="POST" enctype="multipart/form-data" class="mt-6">
            @csrf
            @method('PUT')
                ...
                <div class="px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title',$expense->title) }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="e.g., New Equipment Purchase">
                            @error('title')
                        ...
                        <div>
                            ...
                            <input type="number" name="amount" id="amount" value="{{ $expense->amount }}" required min="0" step="0.01" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            @error('amount')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="expense_date" id="expense_date" value="{{ $expense->expense_date }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            @error('expense_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="category" id="category" required class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                ...
                                <option value="supplements" {{ $expense->category === 'supplements' ? 'selected' : '' }}>Supplements</option>
                        ...
                        <div>
                            ...
                            <input type="text" name="vendor_name" id="vendor_name" value="{{ $expense->vendor_name }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="e.g., ABC Fitness Equipment">
                            @error('vendor_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
